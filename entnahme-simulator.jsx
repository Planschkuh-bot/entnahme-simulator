...
function buildMonthlyGoldCash() {
  ...
  for (const row of relevant) {
    ...
    const bM = Math.pow(1.025 / (1 + row.cpi / 100), 1 / 12) - 1;
    for (let i = 0; i < 12; i++) { goldM.push(gM); cashM.push(cM); bondM.push(bM); }
  }
  return { goldM, cashM, bondM };
}
const { goldM: MONTHLY_GOLD_SP, cashM: MONTHLY_CASH_SP, bondM: MONTHLY_BOND_SP } = buildMonthlyGoldCash();
...
// Rentenabschlag (vorzeitiger Bezug) bzw. -zuschlag (Aufschub) ggü. der Regelaltersgrenze 67:
// -0,3%/Monat vor 67, +0,5%/Monat nach 67 (vereinfachte Nachbildung der gesetzlichen Regelung).
...
function runMonteCarlo(p) {
  seedRng(p.seed);
  const { eq, gold, cash, bond, rhoEqGold, rhoEqBond } = ASSUMPTIONS;
  ...
  const bondMuM = Math.pow(1 + bond.mu, 1 / 12) - 1, bondSigM = bond.sig / Math.sqrt(12);
  ...
  const muLogM = BTC_MU_LOG / 12, sigLogM = BTC_SIG_LOG / Math.sqrt(12);
  const wEq = p.aktien / 100, wGold = p.gold / 100, wCash = p.cash / 100, wBond = p.anleihen / 100, wBtc = p.bitcoin / 100;
  const rhoBtc = BTC_ASSUMPTIONS.rhoWithEq;
  const horizonM = p.horizon * 12;
  const paths = [];
  for (let s = 0; s < p.numSims; s++) {
    const eqR = [], goldR = [], cashR = [], bondR = [], btcR = [];
    for (let t = 0; t < horizonM; t++) {
      ...
      bondR.push(bondMuM + bondSigM * (rhoEqBond * z1 + Math.sqrt(1 - rhoEqBond * rhoEqBond) * z2));
      btcR.push(Math.exp(muLogM + sigLogM * (rhoBtc * z1 + Math.sqrt(1 - rhoBtc * rhoBtc) * z4)) - 1);
    }
    paths.push(simulateWithdrawalBuckets(p.vermoegen, eqR, goldR, cashR, bondR, btcR, wEq, wGold, wCash, wBond, wBtc, p));
  }
...
const MONTHLY_BOND_WORLD = MONTHLY_CASH_WORLD.map((_, i) => {
  const row = HIST_WORLD[Math.floor(i / 12)];
  return Math.pow(1.025 / (1 + row.cpi / 100), 1 / 12) - 1;
});
...
function runHistoricalRolling(p) {
  ...
  for (let start = 0; start + horizonM <= totalM; start++) {
    ...
    const bondR = MONTHLY_BOND_SP.slice(start, start + horizonM);
...
function runWorldRolling(p) {
  ...
  for (let start = 0; start + horizonM <= totalM; start++) {
    ...
    const bondR = MONTHLY_BOND_WORLD.slice(start, start + horizonM);
    const btcR = generateBtcReturnsMonthly(horizonM);
    paths.push(simulateWithdrawalBuckets(p.vermoegen, eqR, goldR, cashR, bondR, btcR, wEq, wGold, wCash, wBond, wBtc, p));
  }
  return aggregate(paths, p.horizon);
...
function runHistoricalBootstrap(p) {
  ...
  const srcBond = p.bootstrapSource === 'world' ? MONTHLY_BOND_WORLD : MONTHLY_BOND_SP;
  const horizonM = p.horizon * 12;
  const blockLenM = p.blockLen * 12;
  const paths = [];
  for (let s = 0; s < p.numSims; s++) {
    const eqR = [], goldR = [], cashR = [], bondR = [];
    while (eqR.length < horizonM) {
      ...
      for (let i = 0; i < blockLenM && eqR.length < horizonM; i++) {
        ...
        eqR.push(srcEq[idx]); goldR.push(srcGold[idx]); cashR.push(srcCash[idx]); bondR.push(srcBond[idx]);
      }
    }
    const btcR = generateBtcReturnsMonthly(horizonM);
    paths.push(simulateWithdrawalBuckets(p.vermoegen, eqR, goldR, cashR, bondR, btcR, wEq, wGold, wCash, wBond, wBtc, p));
...
export default function EntnahmeSimulator() {
  ...
  const [anleihen, setAnleihen] = useState(15);
  const cash = Math.max(0, 100 - aktien - gold - anleihen - bitcoin - wohnung);

  const [staticRate, setStaticRate] = useState(10);
  ...
  const [rentenpunkte, setRentenpunkte] = useState(40);
  ...
  function applyPreset(preset) {
    ...
    const values = preset === 'rentner'
      ? { vermoegen: 300000, aktien: 80, gold: 0, bitcoin: 0, anleihen: 15, wohnung: 0, etfTer: 0.2, staticRate: 8, dynStartRate: 12, ceiling: 5, floor: -2.5, horizon: 25, blockLen: 7, bootstrapSource: 'sp500', startAge: 67, rentenpunkte: 40, pensionStartAge: 67 }
      : { vermoegen: 1150000, aktien: 87, gold: 7, bitcoin: 0, anleihen: 0, wohnung: 0, etfTer: 0.2, staticRate: 2.5, dynStartRate: 3.2, ceiling: 5, floor: -2.5, horizon: 45, blockLen: 7, bootstrapSource: 'sp500', startAge: 50, rentenpunkte: 30, pensionStartAge: 67 };
  ...
  const firstPensionYearTotalIncome = pensionAnnual + firstPensionYearRental + firstPensionYearPortfolioIncome;
  const pensionIncomeShare = firstPensionYearTotalIncome > 0 ? (pensionAnnual / firstPensionYearTotalIncome) * 100 : 0;
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
            ...
            <select value={selectedPreset} onChange={(e) => { if (e.target.value) applyPreset(e.target.value); }} style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <div>
            ...
            <Slider label="Alter bei Entnahmebeginn" value={startAge} min={40} max={70} step={1}
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 220, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 18px' }}>
              ...
              <Stat label="GRV-Rente (pro Jahr)"
                value={fmtEUR(heutigeKaufkraft ? pensionAnnual : pensionAnnual * Math.pow(1 + inflationRate / 100, pensionStartYear))}
                sub={`${heutigeKaufkraft ? `ab Alter ${pensionStartAge}` : `nominal ab Alter ${pensionStartAge}`} · ${pensionIncomeShare.toFixed(1)}% des Gesamteinkommens im 1. GRV-Jahr`}
                color="#3A6B8A"
                tip="Jährlicher Betrag der gesetzlichen Rente. Der Prozentwert zeigt ihren Anteil am Gesamteinkommen im ersten Jahr des Rentenbezugs, zusammen mit Portfolioentnahme und Mieteinnahmen." />
